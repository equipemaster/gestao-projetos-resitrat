...
- **Non-blocking load**: `loadData()` calls `loadThreeDPreviewsAndRender()` without awaiting it — rendering a PDF page (signed URL round trip + pdf.js parse + canvas draw) can take seconds per project, and that used to block every other slide (including Em Andamento) from rendering until all 3D previews finished. Now the other slides render and `refreshActiveSlides()`/`autoScrollActiveSlide()` run immediately; `loadThreeDPreviewsAndRender()` patches in `renderThreeDSlide()` + re-runs `refreshActiveSlides()` whenever the previews finish, in the background. `isVisible3d(p)` (slide membership) only depends on `project_3d_pdf_path` being set, not on the preview image being rendered yet, so `threeDCardHtml()`'s placeholder-icon fallback (see below) can briefly show if the carousel reaches slide-3d before the async render completes.
